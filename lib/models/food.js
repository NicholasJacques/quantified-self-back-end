...
function create(name, calories){
  return database.raw(
    'insert into foods (name, calories, active) values (?, ?, ?) returning *',
    [name, calories, 'true'])
}
...
function update(id, params) {
    if (params.name && params.calories) {
      return database.raw('UPDATE foods SET name = ?, calories = ? WHERE id = ?', [params.name, params.calories, id])
    } else if (params.name) {
      return database.raw('UPDATE foods SET name = ? WHERE id = ?', [params.name, id])
    } else if (params.calories) {
      return database.raw('UPDATE foods SET calories = ? WHERE id = ?', [params.calories, id])
    }
}
